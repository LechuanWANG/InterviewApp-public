...
exports.normalizeAnnotations = normalizeAnnotations;
exports.summarizeAnnotations = summarizeAnnotations;
exports.normalizeRoundReviews = normalizeRoundReviews;
...
function normalizeAnnotations(value, rounds) {
    if (!Array.isArray(value))
        return [];
    const accepted = [];
    const perRoundCounts = new Map();
    const occupiedRanges = new Map();
    for (const rawItem of value) {
        if (accepted.length >= 40)
            break;
...
function summarizeAnnotations(annotations, roundCount) {
    return Array.from({ length: roundCount }, (_, index) => {
        ...
        return {
            ...
            weaknesses: roundAnnotations.filter((item) => item.type === "weakness" || item.type === "clarity" || item.type === "missing").length,
...
function normalizeRoundReviews(value, roundCount) {
    if (!Array.isArray(value))
        return [];
    const accepted = [];
    ...
    for (const rawItem of value) {
        ...
        if (!roundIndex || seenRounds.has(roundIndex))
            continue;
...
function toRoundIndex(value, roundCount) {
    const numeric = typeof value === "number" ? value : typeof value === "string" ? Number(value) : NaN;
    if (!Number.isInteger(numeric) || numeric < 1 || numeric > roundCount)
        return null;
    return numeric;
}
function toAnnotationType(value) {
...
function toDimensions(value) {
    if (!Array.isArray(value))
        return [];
    const dimensions = value.filter((item) => types_1.REPORT_DIMENSIONS.includes(item));
    return Array.from(new Set(dimensions));
}
function toMbtiLetters(value) {
    if (!Array.isArray(value))
        return [];
    return Array.from(new Set(value.filter((item) => MBTI_LETTERS.includes(item))));
}
function findQuoteRange(answer, quote) {
    if (!answer || !quote)
        return null;
    ...
    if (normalizedQuote.text.length < 2)
        return null;
    const normalizedStart = normalizedAnswer.text.indexOf(normalizedQuote.text);
    if (normalizedStart >= 0) {
        return normalizedRangeToOriginalRange(normalizedAnswer, normalizedStart, normalizedStart + normalizedQuote.text.length);
    }
    const orderedSegmentMatch = findOrderedSegmentRange(normalizedAnswer, normalizedQuote.text);
    if (orderedSegmentMatch)
        return orderedSegmentMatch;
...
function normalizedRangeToOriginalRange(normalizedAnswer, startInclusive, endExclusive) {
    ...
    if (start === undefined || lastOriginalIndex === undefined)
        return null;
    return { start, end: lastOriginalIndex + 1 };
}
function findOrderedSegmentRange(normalizedAnswer, normalizedQuote) {
    if (normalizedQuote.length < 18)
        return null;
    ...
    if (normalizedSpanLength > Math.max(12, normalizedQuote.length * 1.8))
        return null;
    return originalRange;
}
function buildOrderedSegments(text) {
...
function findFuzzyQuoteRange(normalizedAnswer, normalizedQuote) {
    ...
    for (let size = minWindow; size <= maxWindow; size += 1) {
        for (let start = 0; start + size <= normalizedAnswer.text.length; start += 1) {
            ...
            if (score < fuzzyThreshold(quoteLength))
                continue;
...
function findPartialQuoteRange(normalizedAnswer, normalizedQuote) {
...
function allowedEditDistance(length) {
    if (length <= 10)
        return 1;
    if (length <= 20)
        return 2;
    if (length <= 50)
        return Math.floor(length * 0.22);
