export type Persona = "warm_hr" | "pro_expert" | "stress" | "deep_tech";
export type Difficulty = "easy" | "medium" | "hard" | "realistic";
export type PersonaInterviewType = "hr" | "technical" | "behavioral" | "mixed";
...
- 语言特征：常用"好的"、"能稍微展开说说吗"、"听起来挺有意思"、"那你当时是怎么想的"`,
  },
  {
    id: "pro_expert",
    label: "专业型",
    description: "中立结构化，STAR 追问，关注业务与技术细节",
    styleHint: `你是资深专业面试官。
- 语气：中立、结构化、理性
- 关注：项目深度、STAR 结构、业务场景理解、技术实现细节、决策过程
- 追问风格：每个关键点都要求具体例子、业务背景、实现细节与权衡依据
- 语言特征：常用"能举个具体例子吗"、"这个业务场景是什么"、"你具体是怎么实现的"、"你当时的权衡是什么"、"如果重来你会怎么做"`,
  },
  {
    id: "stress",
    label: "压力型",
...
- 整场预期 6-8 个主要问题`,
  },
  {
    id: "hard",
    label: "困难",
    description: "高频追问、挑战问题、证据与细节要求",
    answerTimeSec: 240,
    maxRounds: 8,
    hint: `本场难度：困难。
- 高频追问（每个主题 2-3 次追问），对每个关键点都要求事实证据、业务背景和实现细节
...
const PERSONAS_BY_INTERVIEW_TYPE: Record<PersonaInterviewType, Persona[]> = {
...
export function findPersona(id?: string | null): PersonaDef {
  return PERSONAS.find((p) => p.id === id) ?? PERSONAS[1];
}

export function getAllowedPersonasForInterviewType(
  interviewType?: string | null
): PersonaDef[] {
  const key = toPersonaInterviewType(interviewType);
  const allowedIds = PERSONAS_BY_INTERVIEW_TYPE[key];
  return PERSONAS.filter((persona) => allowedIds.includes(persona.id));
}

export function coercePersonaForInterviewType(
  persona?: string | null,
  interviewType?: string | null
): Persona {
...
export function findDifficulty(id?: string | null): DifficultyDef {
  return DIFFICULTIES.find((d) => d.id === id) ?? DIFFICULTIES[1];
}

export function getAnswerTimeLimitSec(id?: string | null): number {
  return findDifficulty(id).answerTimeSec;
}

export function getMaxInterviewRounds(id?: string | null): number {
  return findDifficulty(id).maxRounds;
}

function toPersonaInterviewType(value?: string | null): PersonaInterviewType {
