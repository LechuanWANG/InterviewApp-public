...
} from "./annotateAnswersCore";

export type RoundAnnotationResult = {
  roundReviews: RoundReview[]; // 0 或 1 条(本轮)
  answerAnnotations: AnswerAnnotation[];
};

/**
 * 逐轮批改：每次只批改【一轮】回答，单独调用一次 LLM，返回这一轮的整体点评 + 局部批注。
 * 这样报告页可以「每完成一轮就立即呈现一轮」，而不是等全部轮次一起返回。
 */
export async function annotateRound(
  session: Session,
  report: Report,
  roundIndex: number // 1-based
): Promise<RoundAnnotationResult> {
  const round = session.rounds[roundIndex - 1];
  if (!round) return { roundReviews: [], answerAnnotations: [] };

  if (report.reportKind === "mbti" && report.mbtiReport) {
    return annotateMbtiRound(session, report, round, roundIndex);
  }
  return annotateScoreRound(session, report, round, roundIndex);
}

/**
 * 向后兼容：一次性批改所有轮次(并发逐轮调用后聚合)。
 * 仍被 demo 脚本与 /api/report-demo 使用；线上报告流程改用 annotateRound 逐轮流式呈现。
 */
export async function annotateAnswers(
  session: Session,
  report: Report
): Promise<{
  ...
  const results = await Promise.all(
    session.rounds.map((_, index) => annotateRound(session, report, index + 1))
  );
  const roundReviews = results
    .flatMap((result) => result.roundReviews)
    .sort((a, b) => a.roundIndex - b.roundIndex);
  const answerAnnotations = results.flatMap((result) => result.answerAnnotations);
  return {
    roundReviews,
    answerAnnotations,
    annotationSummaries: summarizeAnnotations(answerAnnotations, session.rounds.length),
  };
}

async function annotateScoreRound(
  session: Session,
  report: Report,
  round: Round,
  roundIndex: number
): Promise<RoundAnnotationResult> {
  const llm = annotationLLM(session);
  const lang = session.language === "zh" ? "中文" : "English";
  const languageRule = annotationLanguageRule(session.language);
  const dimensionList = REPORT_DIMENSIONS.join("、");
  const dimensionSummary = REPORT_DIMENSIONS.map(
    (dimension) => `${dimension}: ${report.dimensionScores[dimension] ?? "未评分"}`
  ).join("、");

  const history = `Round ${roundIndex}${round.isFollowUp ? " (followup)" : ""}${round.timedOut ? " (timed out)" : ""}
Q: ${round.question}
A: ${round.answer}`;
...
你要对候选人【这一轮】回答做整体批改，并辅以少量局部标注，帮助用户理解这一轮回答的逻辑是否成立、是否真正答到了点上。
...
- 只有当口语化表达已经明显影响理解、逻辑表达、业务价值判断、关键能力可信度或面试观感时，才允许指出
...
- 如果本轮回答中同时存在亮点和不足，必须同时标出，不要只标亮点
- 如果本轮回答整体比较一般，允许只有 weakness / suggestion / missing，而没有 strength
- 如果本轮回答的主要问题是“整体结构散、重点不明、没有真正回答问题”，请优先在 overallComment / mainIssue 中指出，而不是集中在个别句子的措辞
...
你只批改第 ${roundIndex} 轮；roundReviews 与 answerAnnotations 中的 roundIndex 必须都等于 ${roundIndex}。
  ...
  "answerAnnotations": [
    {
      ...
      "dimensions": ["${REPORT_DIMENSIONS.join(" | ")}"],
...
额外要求：
- 必须给出这一轮的 roundReviews(恰好 1 条)
- 这一轮尽量至少检查这三类内容：亮点、问题、补充建议，但允许“整体没有明显亮点”
- 不要默认每轮都有亮点；如果没有明显亮点，可以不输出 strength
- 如果回答缺少结果、缺少个人贡献、缺少岗位关联、缺少业务价值表达、缺少关键能力证据、缺少复盘，请优先标出这些问题
- 如果回答虽然句子通顺，但整体没有正面回应问题、结构松散、论点和例子对不上，也应明确判为整体问题
- 对于局部标注，优先选择能支撑整体结论的代表性片段；不要求必须是整句
- 如果这一轮回答信息量较大，请多标几个关键片段，覆盖亮点、问题和缺失，而不是只标 1~2 处
- 你的批改目标不是鼓励性点评，而是帮助候选人提高下一次面试表现，因此请略微严格一些，但保持公正和具体`;
  ...
  return {
    answerAnnotations: normalizeAnnotations(raw.answerAnnotations, session.rounds),
    roundReviews: normalizeRoundReviews(raw.roundReviews, session.rounds.length),
  };
}

async function annotateMbtiRound(
  session: Session,
  report: Report,
  round: Round,
  roundIndex: number
...
你不是在做传统五维评分，也不是在给修改建议。你的任务是针对【这一轮】回答：
- 对这一轮回答做整体点评，说明这一轮主要体现了哪些 MBTI 行为倾向。
...
function annotationLanguageRule(language: Session["language"]): string {
  if (language === "en") {
...
function annotationLLM(session: Session) {
  return {
    provider: session.provider,
    model: session.provider === "deepseek" ? "deepseek-v4-pro" : session.model,
